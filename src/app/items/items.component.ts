...
rowData: any;
  ...
  updateItem(){
    ...
    updatedItem.$id = this.updateFields.idUpdate.value;
    ...
    updatedItem.$genre = this.updateFields.genreUpdate.value;
  ...
  createItem(){
    ...
    createdItem.$genre = this.createFields.genreCreate.value;
    console.log("Create item: "+ createdItem.genre);
    ...
    this.itemService.getTargetItem(id).subscribe( //get the stuff
      resp=>{ //take the response (should be a json)
          ...
          if(resp.status == 200){ 
            ...
            targetItem.$id = this.tgtItem[0].id; //FIX THIS depending on what backend sends back
            ...
            targetItem.$genre = this.tgtItem[0].genre;
